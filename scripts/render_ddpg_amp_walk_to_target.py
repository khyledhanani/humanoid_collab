"""Render trained single-agent DDPG walk-to-target checkpoint."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render DDPG walk_to_target checkpoint in humanoid_collab."
    )
    parser.add_argument("--checkpoint", type=str, default=None)
    parser.add_argument(
        "--checkpoint-dir",
        type=str,
        default="checkpoints/ddpg_amp_walk_to_target",
...
def main() -> None:
    ...
    if "actor" not in ckpt:
        raise KeyError(
            ...
            "Use a checkpoint from scripts/train_ddpg_amp_walk_to_target.py."
        )
    ...
    actor = Actor(obs_dim=obs_dim, act_dim=act_dim, hidden_size=hidden_size)
    ...
    try:
        for ep in range(args.episodes):
            ...
            while not done:
                ...
                if args.noise > 0.0:
                    action = action + rng.normal(0.0, args.noise, size=action.shape)
